...
export default function App() {
  ...
  const predict = async () => {
    ...
    try {
      const response = await axios.get("http://192.168.86.210:5000/students");
      setPrediction({
  ...
  return (
    <View style={styles.container}>
      ...
      {hasUploaded && <Button onPress={predict} title="Predict" />}
      {hasUploaded && (
        <View>
          <Text>Cat: {prediction.cat}</Text>
          <Text>Dog: {prediction.dog}</Text>
        </View>
      )}
    </View>
  );
